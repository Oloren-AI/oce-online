...
def test_basic_slf2(example_data):
    model = RandomForestModel(
        OlorenCheckpoint("default", num_tasks=2048,), n_estimators=10, normalization=QuantileTransformer(),
    )
    train_predict_slf(model, example_data)
...
def test_torchgeometric_save_load(example_data):
    ...
    model = oce.BaseTorchGeometricModel(GINModel(hidden=1, layers=1, batch_size=10), epochs=1)
    train_predict_sl(model, example_data)
...
def test_boost_file(example_data):
    model = oce.BaseBoosting(
        [
            oce.RandomForestModel(oce.DescriptastorusDescriptor("rdkit2dnormalized"), n_estimators=10),
...
def test_molnet_single_task():
    ...
    MTC = oce.RandomForestModel(oce.DescriptastorusDescriptor("morgan3counts"), max_features="log2", n_estimators=1)
    RDK = oce.RandomForestModel(oce.FragmentIndicator(), max_features="log2", n_estimators=1)
    models = [MTC, RDK]
