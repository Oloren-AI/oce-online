...
def test_run_scaffold(example_data3):
    ...
    splitter = oce.ScaffoldSplit(scaffold_filter_threshold=1, split_type='kmeans_murcko', split_proportions=[0.8, 0.1, 0.1])
    for i in splitter.split(example_data3):
        assert(isinstance(i, pd.DataFrame))
        assert(len(i) > 0)
...
def test_split_props_scaffold_murcko(example_data3):
    ...
    splitter = oce.ScaffoldSplit(scaffold_filter_threshold=1, split_type='murcko', split_proportions=split_proportions_TT)
    split = splitter.split(example_data3)
...
def test_split_props_property(example_data1):
    ...
    splitter = oce.PropertySplit(property_col='pChEMBL Value', threshold=None, noise=0.1, categorical=False, split_proportions=split_proportions)
    split = splitter.split(example_data1)
    ...
    splitter = oce.PropertySplit(property_col='pChEMBL Value', threshold=None, noise=0.1, categorical=False, split_proportions=split_proportions_TT)
    split = splitter.split(example_data1)
...
def test_save_load(example_data1, example_data3, tmp_path):
    ...
    stratified = oce.StratifiedSplitter(split_proportions=[0.8, 0.0, 0.2], value_col='pChEMBL Value')
    scaffold = oce.ScaffoldSplit(scaffold_filter_threshold=1, split_type='murcko', split_proportions=[0.8, 0.1, 0.1])
    scaffold_kmeans = oce.ScaffoldSplit(scaffold_filter_threshold=1, split_type='kmeans_murcko', split_proportions=[0.8, 0.1, 0.1])
    property = oce.PropertySplit(property_col='pChEMBL Value', threshold=None, noise=0.1, categorical=False, split_proportions=[0.8, 0.1, 0.1])

    splitters = {'Random':random, 'Stratified':stratified, 'Scaffold':scaffold, 'Scaffold_KMeans':scaffold_kmeans, 'Property':property}
