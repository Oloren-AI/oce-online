...
def test_manage():
    ...
    if os.path.exists("tmp.oce"):
        os.remove("tmp.oce")
    manager = oce.ModelManager(dataset, ["Root Mean Squared Error"], file_path="tmp.oce")
    manager.run(oce.RandomForestModel(oce.DescriptastorusDescriptor("morgan3counts"), n_estimators=10))
    manager.run(oce.MLP(oce.DescriptastorusDescriptor("morgan3counts"), n_estimators=10))
...
def test_sheets_manager():
    ...
    if os.path.exists("tmp.oce"):
        os.remove("tmp.oce")
    manager = oce.SheetsModelManager(dataset, ["Root Mean Squared Error"], file_path="tmp.oce")
    manager.run(oce.RandomForestModel(oce.DescriptastorusDescriptor("morgan3counts"), n_estimators=10))
    manager.run(oce.MLP(oce.DescriptastorusDescriptor("morgan3counts"), n_estimators=10))
    manager2 = oce.load("tmp.oce")
