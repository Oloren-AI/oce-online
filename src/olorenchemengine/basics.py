...
class AutoRandomForestModel(BaseSKLearnModel, BaseObject):
    ...
    @log_arguments
    def __init__(self, representation, n_iter=100, scoring=None, verbose=2, cv=5, **kwargs):
        from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier

        regressor = self.autofit(RandomForestRegressor(), n_iter, cv, scoring, verbose)
        classifier = self.autofit(RandomForestClassifier(), n_iter, cv, scoring, verbose)
        super().__init__(representation, regressor, classifier, log=False, **kwargs)
...
try:
    ...
    mock_imports(globals(), "pl")

try:
    import torch.nn as nn
    import torch
except ImportError:
    mock_imports(globals(), "nn", "torch")
...
class ZWK_XGBoostModel(BaseSKLearnModel, BaseObject):
    ...
    def __init__(self, representation, n_iter=100, scoring=None, verbose=2, cv=5, **kwargs):
        ...
        regressor = self.autofit(XGBRegressor(tree_method="gpu_hist"), n_iter, cv, scoring, verbose)
        classifier = self.autofit(XGBClassifier(tree_method="gpu_hist"), n_iter, cv, scoring, verbose)
...
class SupportVectorMachine(BaseSKLearnModel):
    ...
    @log_arguments
    def __init__(self, representation, C=0.8, kernel="rbf", gamma="scale", coef0=0, cache_size=500, **kwargs):
        regressor = SVR(
            C=C,
            kernel=kernel,
            gamma=gamma,
...
class FeaturesClassification(BaseModel):
    ...
    def _fit(self, X_train, y_train):
        self.numeric_cols = [c for c in X_train.columns.tolist() if is_numeric_dtype(X_train[c])]
