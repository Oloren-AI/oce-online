...
class RemoteSession():
    ...
    def construct(self, object):
        if issubclass(type(object), RemoteObject):
            ...
            if _is_object(object.REMOTE_BC_KEY):
                ...
                return self.get_attribute(object.REMOTE_PARENT, object.REMOTE_BC_KEY.split('.')[-1])
        elif (
            object is None
    ...
    def run_method(self, object, method_name, args, kwargs):
        ...
        print(f"RUN {object.REMOTE_ID}.{method_name} with args = {args}, and kwargs = {kwargs}")

    def get_attribute(self, object, attribute_name):
        object = self.construct(object)
        object.attribute_name.REMOTE_ID = generate_random_uuid()
        print(f"GET_ATTR {object.REMOTE_ID}.{attribute_name} => {object.attribute_name.REMOTE_ID}")
...
class RemoteObject():
    ...
    def __getattribute__(self, key):
        EXCLUDED_WORDS = ["REMOTE_BC_KEY", "REMOTE_SESSION", "REMOTE_PARENT", "REMOTE_ID", "REMOTE_CHILDREN", "args", "kwargs", "shabalaba"]
        ...
        try:
            ...
            if key in self.REMOTE_CHILDREN:
                ...
                self.REMOTE_CHILDREN[key] =  RemoteObject(self.REMOTE_BC_KEY + "." + key, self, self.REMOTE_SESSION)
                return self.REMOTE_CHILDREN[key]
