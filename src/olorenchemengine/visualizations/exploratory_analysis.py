    ...
    def __init__(self, dataset: BaseDataset, features: list = [], log=True, **kwargs):
        ...
        for feature in self.features:
            if isinstance(feature, str):
                ...
                self.spearman_corr.append(
                    spearmanr(
                        self.d[feature],
                        self.dataset.data[self.dataset.property_col].tolist(),
                    )
                )
                ...
                try:
                    ...
                    names = [
                        f"{feature.__class__.__name__}-{i}"
                        for i in range(rep_features.shape[0])
                    ]
                for i, name in enumerate(names):
                    ...
                    self.spearman_corr.append(
                        spearmanr(
                            self.d[name], self.dataset.data[self.dataset.property_col]
                        )
                    )

    def get_data(self):
        d = {
            "datacols": self.d.copy(),
