...
pkatable_grimsley = [
    (
        ...
        Chem.MolFromSmiles(
            "CN(C)C1=C-C=C2-C(C4=C(C(=O)O)C=CC=C4)=C3-C=C-C(=[N+](C)C)C=C3OC2=C1"
        ),
        [-10, 1.19, 3.59, 10],
        [1, 1, 0, 1],
    ),
    (
        ...
        Chem.MolFromSmarts("c2c3ccc(O)cc3oc3cc(=O)ccc2-3"),
        [2.94, 6.56],
        [1, 0],
    ),
    ...
    (
        "N-acetyl Piperazine",
        Chem.MolFromSmarts("[N+&!H0,NX3&H0;$(N1(C)CCN(C(=O)C)CC1)]"),
        7.1,
        1,
    ),  # Reaxys
    ...
    (
        "Phosphonate",
        Chem.MolFromSmarts("[OH]P([OH])(=O)[#6]"),
        [1.5, 7.0],
        [0, -1],
    ),  # Guess
    (
        "7H-Pyrimido[4,5-b][1,4]oxazine-2,4-diamine",
        Chem.MolFromSmarts("C1C=NC2=CN=C(N=C2O1)N"),
        3.6,
        1,
    ),  # PKA Guessed
    ...
    (
        "Glutamic Acid",
        Chem.MolFromSmarts("[$([CX3][CD2][CD2]C(N)C(=O))](=O)[OX1H0-,OX2H1]"),
        4.2,
        0,
    ),
    ...
    (
        "Condensed Cysteine",
        Chem.MolFromSmarts("[$([SH][#175])]"),
        6.8,
        0,
    ),  # Sidechain treated as normal atom
    (
        "Condensed Cysteine",
        Chem.MolFromSmarts("[$([SH][#178])]"),
        6.8,
        0,
    ),  # Sidechain treated as normal atom
    ...
    (
        "Aspartic Acid v2",
        Chem.MolFromSmarts("[$([CX3][CD2]C(N)C(=O))](=O)[OX1H0-,OX2H1]"),
        3.5,
        0,
    ),
    ...
    (
        "End Carboxylate (avg)",
        Chem.MolFromSmarts("[$([CX3]CN)](=O)[OX1H0-,OX2H1]"),
        3.3,
        0,
    ),
    # ('End Amine (avg)',Chem.MolFromSmarts('[$([NX3;H2,H1;!$(NC=O)]CC=O)]'),9.15,1),
    (
        "End Amine (avg) PseudoAtom safe",
        Chem.MolFromSmarts(
    ...
    (
        "Generic tert. Amine, not amide",
        Chem.MolFromSmarts("[N+&!H0&NX3,NX3;$(N(C)(C)C);!$(NC=[!#6]);!$(NC#[!#6])]"),
        9.8,
        1,
    ),
    (
        "Generic Amine, not amide",
        Chem.MolFromSmarts(
...
pkatable_ipc = [
    ...
    (
        "Lysine",
        Chem.MolFromSmarts("[$([ND1][CD2][CD2][CD2][CD2]C(N)C(=O))]"),
        10.517,
        1,
    ),
    (
...
def find_pKas(
    ...
    dummy = Chem.MolFromSmarts(
        "[#178]"
    )  # Used Gly instead of #0 or *, for compatibility with building blocks
    ...
    if returnindices:
        ...
        for atom in mol.GetAtoms():
            atom.SetProp(
                "O_i", str(atom.GetIdx())
            )  # Save the Original Idx as a property
        ...
        if mol.HasSubstructMatch(s[1]):
            ...
            if returnindices:
                for match in substructmatches:
                    atomindices += [
                        [int(mol.GetAtomWithIdx(idx).GetProp("O_i")) for idx in match]
                    ]  # Retrieve the original index of matches
            mol = AllChem.ReplaceSubstructs(mol, s[1], dummy)[
                0
            ]  # Delete acidic/basic groups to prevent multiple matching
...
def pI(pkalist, chargelist):
    ...
    if len(unique(chargelist)) == 0:  # No pKa groups present.
        ...
        pI = (
            argmin(np_abs(chargecol)) / 100.0
        )  # Simply taking the smallest absolute value, and dividing the index with 100
    # print "pI %.1f"%pI
    return pI
...
class calc_pI(BaseCompoundVecRepresentation):
    ...
    def __init__(self, ph=7.4, log=True, **kwargs):
        self.ph = ph
        super().__init__(
            names=["pI", f"Charge{self.ph}", f"TotalAbsCharge{self.ph}"],
            log=False,
            **kwargs,
        )
