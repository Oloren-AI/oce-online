...
class MolCLR_BF(BondFeaturizer):
    ...
    def convert(self, bond: Chem.Bond):
        edge_feature = [BOND_LIST.index(bond.GetBondType()), BONDDIR_LIST.index(bond.GetBondDir())]
...
class MolCLR(BaseModel):
    ...
    def _fit(self, X, y=None, **kwargs):
        ...
        base_params = list(
            map(lambda x: x[1], list(filter(lambda kv: kv[0] not in layer_list, self.model.named_parameters())))
        )
        ...
        train_loader = DataLoader(
            X, batch_size=self.config["batch_size"], shuffle=True, num_workers=oce.CONFIG["NUM_WORKERS"]
        )
    ...
    def _predict(self, X, **kwargs):
        ...
        loader = DataLoader(
            X, batch_size=self.config["batch_size"], shuffle=False, num_workers=oce.CONFIG["NUM_WORKERS"]
        )
        self.model.eval()
        y_pred = []
